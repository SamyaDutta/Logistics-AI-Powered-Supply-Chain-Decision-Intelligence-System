...
# Streamlit Cloud exposes deployment secrets through st.secrets rather than
# process environment variables. Keep local .env usage working as-is.
for setting_name in (
    "GROQ_API_KEY",
    "PINECONE_API_KEY",
    "SQL_SERVER_HOST",
    ...
    if setting_name not in os.environ:
        try:
            os.environ[setting_name] = str(st.secrets[setting_name])
        except KeyError:
            pass
